...
public class DatabaseThread implements Runnable {
	...
	public static String strName = "DatabaseThread ";
	public int iWorkerNumber;
	...
	public DatabaseThread(Database d, HikariDataSource h, int i) {
		ds = h;
		oDB = d;
		oMain = d.oMain;
		iWorkerNumber = i;
			...
			try (Connection con = ds.getConnection();) {
				...
				if (rs.next()) {
					...
					if (!rs.getString("updated_at").equals(o.aStrQuery1[3])
						...
						try (PreparedStatement ps2 = con
								.prepareStatement("UPDATE pools SET updated_at = ?, description = ? WHERE id = ?");) {
	...
	private void getDownloadQueue(DBObject o) {
		try (Connection con = ds.getConnection();) {
			PreparedStatement ps = con.prepareStatement("SELECT * FROM download_queue " + o.strQuery1 + " LIMIT 1");
			ResultSet rs = ps.executeQuery();
			rs = ps.executeQuery();
			if (rs.next()) {
	...
	private void getNewestPosts(DBObject o) {
		try (Connection con = ds.getConnection();
				PreparedStatement ps = con
						.prepareStatement("SELECT id FROM posts WHERE NOT rename_ext = 0 ORDER BY id DESC LIMIT "
								+ o.strQuery2 + " OFFSET " + o.strQuery1);) {
			ResultSet rs = ps.executeQuery();
	...
	private void searchTags(DBObject o) {
		try (Connection con = ds.getConnection();) {
			PreparedStatement ps = con
					.prepareStatement("SELECT consequent_name FROM tag_aliases WHERE antecedent_name = ? LIMIT 1");
			ps.setString(1, o.strQuery1);
			ResultSet rs = ps.executeQuery();
			if (rs.next()) {
